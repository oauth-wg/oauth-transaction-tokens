...
`scope`:
: REQUIRED A String defining the purpose or intent of this transaction. Note that the value of this claim is defined by the Transaction Token Service and is not required to match the requested scope nor the scope in any supplied external token.
